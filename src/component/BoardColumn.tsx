...
function BoardColumn({ columnId, isNewTaskModalOpen, setIsNewTaskModalOpen }: ColumnProps) {
  ...
  const tasksInColumn = useMemo(() => {
    ...
    if (priority !== "All")
      tasksInColumn = tasksInColumn?.filter((task: Task) => task.priority === priority);
    if (storyPoint)
      tasksInColumn = tasksInColumn?.filter((task: Task) => task.storyPoints === storyPoint);
    if (titleSearch)
      tasksInColumn = tasksInColumn?.filter((task: Task) =>
  ...
  return (
    <>
        ...
        <Card.Root w="100%" h="100%">
          <Card.Header>
            <Box display="flex" borderBottom="1px solid gray" pb={2}>
              ...
              <Badge size="sm" bgColor="gray.500" color="white">
      ...
      {createPortal(
        <NewTaskModal isNewTaskModalOpen={isNewTaskModalOpen} onClose={setIsNewTaskModalOpen} />,
